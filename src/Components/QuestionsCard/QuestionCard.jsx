...
const QuestionCard = ({questionsData, score, setScore, setCount, count, modal, setModal}) => {
    ...
    <div className='questionCard'>
        ...
        <div className='questionCard-title'> {count + 1}/10 - {questionsData[count]?.question}</div>
        {
            questionsData[count]?.answers?.map((answer, i) => (
                    <button className='buttonCard' onClick={approvedChoice} key={i} value={answer}>{answer}</button>
            ))
        }
    </div>
  )
